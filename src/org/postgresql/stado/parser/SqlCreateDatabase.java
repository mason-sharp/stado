        ...
        try {
            ...
            try {
                ...
                if (spatial) {
				   ...
				   sql = sqlHeader + "(" + tableID + ", 1, 'f_table_catalog', 12, 256, 0, 0, null, '\"f_table_catalog\" VARCHAR (256) NOT NULL')";
				   MetaData.getMetaData().executeUpdate(sql);
				
				   sql = sqlHeader + "(" + tableID + ", 2, 'f_table_schema', 12, 256, 0, 0, null, '\"f_table_schema\" VARCHAR (256) NOT NULL')";
				   MetaData.getMetaData().executeUpdate(sql);
				
				   sql = sqlHeader + "(" + tableID + ", 3, 'f_table_name', 12, 256, 0, 0, null, '\"f_table_name\" VARCHAR (256) NOT NULL')";
				   MetaData.getMetaData().executeUpdate(sql);
				
				   sql = sqlHeader + "(" + tableID + ", 4, 'f_geometry_column', 12, 256, 0, 0, null, '\"f_geometry_column\" VARCHAR (256) NOT NULL')";
				   MetaData.getMetaData().executeUpdate(sql);
				
				   sql = sqlHeader + "(" + tableID + ", 5, 'coord_dimension', 4, null, 0, 0, null, '\"coord_dimension\" INT NOT NULL')";
				   MetaData.getMetaData().executeUpdate(sql);
	
				   sql = sqlHeader + "(" + tableID + ", 6, 'srid', 4, null, 0, 0, null, '\"srid\" INT NOT NULL')";
				   MetaData.getMetaData().executeUpdate(sql);
	
				   sql = sqlHeader + "(" + tableID + ", 7, 'type', 12, 30, 0, 0, null, '\"type\" VARCHAR (30) NOT NULL')";
				   MetaData.getMetaData().executeUpdate(sql);
	
	                
	               sql = "INSERT INTO xsystables (dbid, tablename, numrows, partscheme, owner)" +
	                             " VALUES (" + dbid + ", 'spatial_ref_sys', 3750, 2, " + ownerid + ") " +
	                             " RETURNING tableid ";
